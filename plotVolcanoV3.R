...
plotVolcano <- function(mat, gene.col, x.col, y.col, x_cut1, x_cut2,
                        ...
                        x.lim, y.lim, title="", label, selected_genes) {
  ...
  #colnames(mat)[colnames(mat) %in% shape] = 'shape'
  ...
  xmin <- -1*x.lim #(range(mat$x.col)[1]- (range(mat$x.col)[1] + x.lim))
  xmax <- 1*x.lim # (range(mat$x.col)[1]+ (x.lim - range(mat$x.col)[1]))
  ymin <- 0
  ymax <- y.lim
  ...
  #set colors
  cols[mat$y.col < y_cut1 & mat$x.col > x_cut1]<- "#9970AB"#"#BF812D"#
  cols[mat$y.col < y_cut2 & mat$x.col > x_cut2]<-  "#762A83"#"#8C510A"#
  cols[mat$y.col < y_cut1 & mat$x.col < -x_cut1]<- "#5AAE61"#"#35978F" #
  cols[mat$y.col < y_cut2 & mat$x.col < -x_cut2]<-  "#1B7837"#"#01665E" #
  ...
  p1 <- p + geom_text_repel(aes(label = label),
                     max.overlaps = 100000, size = 2.5,
                     box.padding = 0.5,
                     #point.padding = unit(0.3, "lines"),
                     segment.color = "black", segment.size = 0.05)
